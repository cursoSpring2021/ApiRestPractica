...
public class ProductoServiceImpl implements ProductoService {
    ...
    @Override
    public List<ProductoDto> getProductoCategoria(String categoria) {

        List<Producto> productos = productoRepository.findByCategoria(categoria);
        if (!productos.isEmpty()) {
            return productoMapper.toProductoDtos(productos);
        } else {
            throw new AppException("No hay producto con esa categoria", HttpStatus.NOT_FOUND);
        }
    ...
    public ProductoDto delete(Long productoId) {
        Producto producto = productoRepository.findById(productoId)
                .orElseThrow(() -> new AppException("Producto not foind", HttpStatus.NOT_FOUND));
